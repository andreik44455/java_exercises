...
public class Date
{
  ...
  public Date addMonth()
  {
    ...
    try { return new Date(day, month + 1, year); }
      ...
      // The day is not exist in the next month
      // Try to find the new last day in the next month
      try { return new Date(daysInMonth(month + 1, year), month + 1, year); }
      catch (Exception exception2)  {
        // Now try the next year.
  ...
  public Date subtractDay() throws Exception
  {
    ...
    try { return new Date(day - 1, month, year); }
      ...
      // Try find out how many days in the previous month.
      try { return new Date(daysInMonth(month - 1, year), month - 1, year); }
      catch (Exception exception2) {
        //Try the last day of the previous year
        try { return new Date(31, 12, year - 1); }
        catch (Exception exception3) { return null; }
      }// catch
    }// catch
  } // subtractDay
  ...
  public Date subtractMonth() throws Exception
  {
    ...
    try { return new Date(day, month - 1, year); }
      ...
      //Try to find out how many days in the previous month.
      try { return new Date(daysInMonth(month - 1, year), month - 1, year); }
      catch (Exception exception2) {
        //Try the last month of the previous year.
  ...
  public int daysFrom(Date other) throws NullPointerException
  {
    ...
    if (equals(other))
      ...
      // We should not get an exception from subtractDay,
